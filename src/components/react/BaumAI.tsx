...
export default function BaumAI() {
	...
	return (
		<div className="flex h-[600px] flex-col text-sm">
					...
					{messages.length === 0 && (
						<motion.div
							...
							<div className="max-w-xs rounded-lg bg-green-100 px-2.5 py-1.5 text-emerald-700">
								Hi! I'm{" "}
								<span className="font-semibold">Jeremy's AI persona</span>. Ask
			...
			<div className="flex w-full justify-between gap-2 pt-4 text-xs">
				<button
					onClick={() => handleButtonClick("What are you currently working on?")}
				...
				<button
					onClick={() => handleButtonClick("Are you available for consulting opportunities?")}
					...
					disabled={isLoading}>
					Are you available for consulting opportunities?
				</button>
				<button
					onClick={() =>
						handleButtonClick(
